...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/burger')
        .then(res => {
            console.log(res.data);
...
/*
    ...
    Inside the repeatMyParam function above, grab the element with the id of 'repeat-text' and set its textContent property equal 
...
// CODE HERE
const getQueryTest = () => {
    axios.get('http://localhost:3000/query-test/?check=mate')
        .then(res =>{
            console.log(res.data)
        });
}

document.querySelector('#query-button').addEventListener('click', getQueryTest);
...
/*
    ...
    Returned: "You sent an empty query!"

    2: Send more than 1 query on the URL -- what happened? 
    Returned message "message: "You sent more than 1 query!"
*/
